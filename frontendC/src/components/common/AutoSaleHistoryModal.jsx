...
import { getAutoSaleHistory, addAutoSaleHistory } from "../../api/autoSaleApi";
import useAuth from "../../hooks/useAuth";
import { useSidebar } from "../../contexts/SidebarContext";
import { cn } from "../../utils/cn";

export const AutoSaleHistoryModal = ({
  isOpen,
  onClose,
  saleId,
  saleIdentifier,
  ...
  const { data: historyData, isLoading: historyLoading } = useQuery({
    queryKey: ["autoSaleHistory", saleId],
    queryFn: () => getAutoSaleHistory(saleId),
    enabled: isOpen && !!saleId,
  });

  const historyMutation = useMutation({
    mutationFn: addAutoSaleHistory,
    onSuccess: () => {
      queryClient.invalidateQueries(["autoSaleHistory", saleId]);
      queryClient.invalidateQueries(["auto-sales"]);
      toast.success("Auto Sale history updated!");
      reset();
      setActiveTab("history");
    },
    onError: (error) => {
      toast.error(
        error.response?.data?.msg || "Failed to update auto sale history",
      );
    },
  });
  ...
  const rejectionOptions = [
    ...
    { value: "auto_warranty_missing", label: "Auto warranty missing" },
    {
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6">
            ...
            <div className="space-y-4">
              ...
              {historyLoading ? (
                ...
                <p className="text-gray-500">
                  No history available for this auto sale.
                </p>
              )}
            </div>
          )}
          ...
          {activeTab === "status" && canUpdateStatus() && (
            <div>
              <h3 className="text-lg font-medium mb-4">
                Update Auto Sale Status & Add Comment
              </h3>
